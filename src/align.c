...
static void mm_filter_bad_seeds_alt(void *km, int as1, int cnt1, mm128_t *a, int min_gap, int max_ext)
{
	...
	for (k = 0; k < n;) {
		...
		for (l = k + 1; l < n; ++l) {
			...
			q_span_pre = a[as1 + j - 1].y >> 32 & 0x3fff;
...
static void mm_fix_bad_ends(const mm_reg1_t *r, const mm128_t *a, int bw, int min_match, int32_t *as, int32_t *cnt)
{
	...
	*as = r->as, *cnt = r->cnt;
	if (r->cnt < 3) return;
	m = l = a[r->as].y >> 32 & 0x3fff;
	for (i = r->as + 1; i < r->as + r->cnt - 1; ++i) {
		int32_t lq, lr, min, max;
		int32_t q_span = a[i].y >> 32 & 0x3fff;
	...
	*cnt = r->as + r->cnt - *as;
	m = l = a[r->as + r->cnt - 1].y >> 32 & 0x3fff;
	for (i = r->as + r->cnt - 2; i > *as; --i) {
		int32_t lq, lr, min, max;
		int32_t q_span = a[i+1].y >> 32 & 0x3fff;
...
static void mm_max_stretch(const mm_reg1_t *r, const mm128_t *a, int32_t *as, int32_t *cnt)
{
	...
	*as = r->as, *cnt = r->cnt;
	...
	score = a[r->as].y >> 32 & 0x3fff, len = 1;
	for (i = r->as + 1; i < r->as + r->cnt; ++i) {
		int32_t lq, lr, q_span;
		q_span = a[i].y >> 32 & 0x3fff;
		lr = (int32_t)a[i].x - (int32_t)a[i-1].x;
		lq = (int32_t)a[i].y - (int32_t)a[i-1].y;
		if (lq == lr) {
...
{
	uint8_t *qseq, *tseq;
	int q_span = a->y>>32&0x3fff, qs, qe, rs, re, rid, score, q_off, t_off, ext_len = opt->anchor_ext_len;
	void *qp;
	rid = a->x<<1>>33;
...
{ // this assumes a very crude k-mer based mode; it is not necessary to use a good model just for filtering bounary exons
	...
	*as1 = r->as, *cnt1 = r->cnt;
	...
	if ((a[r->as].y>>32&0x3fff) < log_gap + opt->anchor_ext_shift) {
	...
	if ((a[r->as + r->cnt - 1].y>>32&0x3fff) < log_gap + opt->anchor_ext_shift) {
...
//        printf("%d", cnt1);
        //reference&query start - query span
		rs = (int32_t)a[as1].x + 1 - (int32_t)(a[as1].y>>32&0x3fff);
		qs = (int32_t)a[as1].y + 1 - (int32_t)(a[as1].y>>32&0x3fff);
        //referenceQquery end
...
//        printf("%d\n", cnt1);
	...
	if (is_sr) {
		...
		// compute rs0 and qs0
		rs0 = (int32_t)a[r->as].x + 1 - (int32_t)(a[r->as].y>>32&0x3fff);
		qs0 = (int32_t)a[r->as].y + 1 - (int32_t)(a[r->as].y>>32&0x3fff);
		...
		for (i = r->as - 1, l = 0; i >= 0 && a[i].x>>32 == a[r->as].x>>32; --i) { // inspect nearby seeds
			int32_t x = (int32_t)a[i].x + 1 - (int32_t)(a[i].y>>32&0x3fff);
			int32_t y = (int32_t)a[i].y + 1 - (int32_t)(a[i].y>>32&0x3fff);
			if (x < rs0 && y < qs0) {
				if (++l > opt->min_cnt) {
