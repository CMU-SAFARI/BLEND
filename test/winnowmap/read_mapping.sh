...
mkdir -p ${OUTDIR}
/usr/bin/time -v -p -o ${OUTDIR}/${PREFIX}.time bash ../scripts/winnowmap-map.sh ${OUTDIR} ${PREFIX} ${READS} ${REF} ${THREAD} ${THREAD_SORT} ${PRESET} > ${OUTDIR}/${PREFIX}.out 2> ${OUTDIR}/${PREFIX}.err
