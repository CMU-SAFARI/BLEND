...
/usr/bin/time -v -p -o "${OUTDIR}/${PREFIX}_winnowmap.time" winnowmap -W "${OUTDIR}/${PREFIX}_repetitive_k15.txt" -ax map-pb -t ${THREAD} ${REF} ${READS} | samtools sort -l5 -m4G -@ ${THREAD_SORT} -o "${OUTDIR}/${PREFIX}_winnowmap.bam"
