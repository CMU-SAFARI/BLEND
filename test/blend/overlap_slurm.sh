...
sbatch -J bo.$PREFIX -o ${OUTDIR}/${PREFIX}.out -e ${OUTDIR}/${PREFIX}.err -c ${THREAD} $SLURM_OPTIONS --wrap="bash ../scripts/blend-overlap.sh ${OUTDIR} ${PREFIX} ${READS} ${PRESETX} ${THREAD}"

PREFIX="Ecoli.PB.HiFi.100X_blendi"
PARAMS="--immediate"
sbatch -J bo.$PREFIX -o ${OUTDIR}/${PREFIX}.out -e ${OUTDIR}/${PREFIX}.err -c ${THREAD} $SLURM_OPTIONS --wrap="bash ../scripts/blend-overlap.sh ${OUTDIR} ${PREFIX} ${READS} ${PRESETX} ${THREAD} \"${PARAMS}\""
